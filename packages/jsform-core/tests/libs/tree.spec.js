...
import { TreeMap } from "../../dist/index.dev";
...
describe("测试treemap", () => {
    ...
    it("移动位置,当前位置大于移动的位置", () => {
        let nbc = tree.contains("b").contains("c");
        let nbc4 = nbc.contains(4), nbc1 = nbc.contains(1);

        nbc4.insertToFromParent(1);

        expect(nbc.contains(1)).to.equal(nbc4);
        expect(nbc.contains(2)).to.equal(nbc1);
    });

    it("移动位置, 移动到超过数组长度的位置", () => {
        let nbc = tree.contains("b").contains("c");
        let nbc4 = nbc.contains(4);

        nbc4.insertToFromParent(10);

        expect(nbc.contains(10)).to.equal(nbc4);
        expect(nbc.contains([ 9 ])).to.equal(null);
    });
